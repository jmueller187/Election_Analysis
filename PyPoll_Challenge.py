...
with open(file_to_save, "w") as txt_file:
    ...
    for county_name in county_votes:
        ...
        county_results = (f"{county_name}: {vote_percentage:.1f}% ({votes_per_county:,})\n")
        print(county_results)
         # 6e: Save the county votes to a text file.
        txt_file.write(county_results)
         # 6f: Write an if statement to determine the winning county and get its vote count.
        if (votes_per_county > top_county_votes) and (vote_percentage > top_county_percentage):
            top_county_votes = votes_per_county
            top_county_percentage = vote_percentage
            top_county_turnout = county_name
